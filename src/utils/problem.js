	...
	problems.forEach((problemName) => {
		...
		if (fs.existsSync(subDir) && fs.statSync(subDir).isDirectory()) {
			...
			if (fs.existsSync(contestPath)) {
				...
				files.forEach((f) => {
					if (
						f.startsWith(problemName + '.') &&
						fs.statSync(path.join(contestPath, f)).isFile()
					) {
...
export const markProblems = (contest, allProblems, selectedProblems) => {
	...
	allProblems.forEach((problemName) => {
		data.ac[contest][problemName] =
			selectedProblems.includes(problemName);
	});
	updateData(data);
};
...
export const renameProblem = (contest, oldName, newName) => {
	...
	if (fs.existsSync(subDir) && fs.statSync(subDir).isDirectory()) {
		...
		files.forEach((f) => {
			if (f.startsWith(oldName + '.')) {
				...
				fs.renameSync(
					path.join(newSubDir, f),
					path.join(newSubDir, newName + ext),
				);
			}
		});
	} else {
		...
		files.forEach((f) => {
			...
			if (
				...
				const ext = path.extname(f);
				fs.renameSync(
					path.join(contestPath, f),
					path.join(contestPath, newName + ext),
				);
			}
		});
	}
	...
	if (fs.existsSync(testDir)) {
		...
		files.forEach((f) => {
			if (
				f.startsWith(oldName + '_') &&
				fs.statSync(path.join(testDir, f)).isFile()
			) {
				const suffix = f.substring(oldName.length);
				fs.renameSync(
					path.join(testDir, f),
					path.join(testDir, newName + suffix),
				);
			}
		});
	}
...
export const moveProblem = (fromContest, toContest, problemName) => {
	...
	if (
		filesInTo.some(
			(f) =>
				f.startsWith(problemName + '.') &&
				fs.statSync(path.join(toPath, f)).isFile(),
		)
	)
		existsInTarget = true;
	...
	if (existsInTarget) {
		throw new Error(
			`Problem ${problemName} already exists in ${toContest}`,
		);
	}

	const subDirFrom = path.join(fromPath, problemName);
	if (
		fs.existsSync(subDirFrom) &&
		fs.statSync(subDirFrom).isDirectory()
	) {
		...
		files.forEach((f) => {
			...
			if (
				f.startsWith(problemName + '.') &&
				fs.statSync(path.join(fromPath, f)).isFile()
			) {
				fs.renameSync(
					path.join(fromPath, f),
					path.join(toPath, f),
				);
			}
		});
	...
	if (fs.existsSync(fromTestDir)) {
		...
		files.forEach((f) => {
			if (
				f.startsWith(problemName + '_') &&
				fs.statSync(path.join(fromTestDir, f)).isFile()
			) {
				fs.renameSync(
					path.join(fromTestDir, f),
					path.join(toTestDir, f),
				);
			}
		});
	}
	...
	if (
		data.ac[fromContest] &&
		data.ac[fromContest][problemName] !== undefined
	) {
		data.ac[toContest][problemName] =
			data.ac[fromContest][problemName];
