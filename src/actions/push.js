	...
	try {
		...
		if (!statusOutput || !statusOutput.trim()) {
			...
			for (const contest of contestChanges) {
				console.log(
					`Adding changes for contest: ${contest}...`.cyan,
				);
				execSync(`git add "${contest}"`, {
					cwd: contestDir,
					stdio: 'ignore',
				});
				...
				try {
					...
					console.log(
						`Failed to commit contest ${contest}`.yellow,
					);
				}
			}
		...
		console.log(
			`Pushing to remote repository (branch ${branchName})...`.cyan,
		);
		...
		console.log(
			`Failed to push to git repository. Please check if your remote is configured correctly and you have push access.`
				.error,
		);
